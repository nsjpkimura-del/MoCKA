...
function Run-SearchGet([string]$Query, [int]$Top) {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "searchget failed with exitcode=$LASTEXITCODE"
  }
}
...
function Run-SafeEdit([string]$Target, [string]$Editor, [string]$Note) {
  ...
  if ([string]::IsNullOrWhiteSpace($Note)) {
    throw "note is required (audit reason)."
  }
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "safe_edit failed with exitcode=$LASTEXITCODE"
  }
}
...
function Run-Restore([string]$Target, [string]$Timestamp) {
  ...
  if ([string]::IsNullOrWhiteSpace($Timestamp)) {
    throw "timestamp is required (YYYYMMDD_HHMMSS)."
  }
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "restore failed with exitcode=$LASTEXITCODE"
  }
}
